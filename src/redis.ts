...
export const DEFAULT_REDIS_CONNECTION_OPTIONS: RedisOptions = {
  ...parseURL(REDIS_URL),
  tls:
    REDIS_URL.startsWith('rediss') && REDIS_CA_CERT ? { ca: Buffer.from(REDIS_CA_CERT) } : undefined
};

export const DEFAULT_REDIS_CACHE_CONNECTION_OPTIONS: RedisOptions = {
...
export const sharedRedisClient = new Redis(DEFAULT_REDIS_CONNECTION_OPTIONS);
export const sharedRedisCacheClient = new Redis(DEFAULT_REDIS_CACHE_CONNECTION_OPTIONS);
...
export async function withRedisClient<R>(
  ...
  const client = new Redis(REDIS_URL, { ...DEFAULT_REDIS_CONNECTION_OPTIONS, ...opts });
  try {
...
export function createRedisClient(opts?: Partial<RedisOptions>) {
  const client = new Redis({ ...DEFAULT_REDIS_CONNECTION_OPTIONS, ...opts });
  CLIENTS.push(client);
  return client;
}
